...
import { editHabit } from "@/src/app/lib/habitSlice";
...
export default function EditHabitModal({ habit, isOpen, onClose }: EditHabitModalProps) {
  ...
  useEffect(() => {
    if (habit) {
      ...
      // Map frequency from habit format to form format
      switch (habit.frequency.type) {
        ...
        case "weekly":
          ...
          // Convert day indices back to day names
          const dayNames = habit.frequency.days?.map(dayIndex => weekdays[dayIndex]) || [];
          setSpecificDays(dayNames);
          break;
        case "twice-a-week":
          setFrequency("twice");
          setTwiceWeekDays(habit.frequency.days || []);
          break;
        case "custom-dates":
          setFrequency("specificDate");
          setSpecificDates(habit.frequency.dates?.map(date => parseInt(date)) || []);
          break;
        default:
          setFrequency("daily");
      }
    } else {
      // Reset form when habit is null
      setName("");
      setGoal("");
      setFrequency("daily");
      setSpecificDays([]);
  ...
  const modalContent = isOpen && modalRoot && habit ? (
    <div 
    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full shadow-xl border dark:border-gray-700 max-h-[90vh] overflow-y-auto max-w-lg lg:max-w-xl">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100">Edit Habit</h2>
          <button
            onClick={onClose}
  ...
  return modalContent && modalRoot ? createPortal(modalContent, modalRoot) : null;
}
